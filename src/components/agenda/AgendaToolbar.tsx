...
const AgendaToolbar = ({
  ...
  selectedDate: Date;
  activeFilter: string;
  setActiveFilter: (v: "all" | "note" | import("@/types").RecordType) => void;
  handleAddNote: () => void;
  handleAddRecord: () => void;
  onDateSelect: (date: Date) => void;
  t: any;
}) => {
  // Yardımcı: Güvenli tip dönüşümü
  const handleTabChange = (v: string) => {
    // 'all', 'note', 'income', 'expense', 'financing' değerleri dışındakiler için koruma
    if (
      v === "all" ||
      v === "note" ||
      v === "income" ||
      ...
      setActiveFilter(v as "all" | "note" | import("@/types").RecordType);
    }
  };
